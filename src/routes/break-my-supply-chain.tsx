...
import { failureToggles, hiddenDependencies } from "@/lib/parallax/data";
        ...
        <Panel>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on ? "border-critical/50 bg-critical/10" : "border-border bg-surface hover:border-border-strong",
                  )}
                >
                  <span
                    className={cn(
                  ...
                  <span>
                    <span className={cn("block text-xs", on ? "text-critical" : "text-foreground")}>{f.label}</span>
                    <span className="block text-[11px] text-muted-foreground">{f.detail}</span>
                  </span>
                  <span className="num ml-auto text-[10px] text-muted-foreground">−{f.resilienceHit}</span>
                </button>
              );
            })}
          </div>
          <div className="border-t border-border p-3">
            ...
            {chaosToggles.length === 0 && !chaosRunning ? (
              <p className="mt-2 text-[11px] text-muted-foreground">Select at least one failure to inject.</p>
            ) : null}
          </div>
        </Panel>
      ...
      {chaosResult ? (
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <Panel className="p-4">
              ...
              <div className="mt-3 flex items-end gap-5">
                <div>
                  ...
                  <p className="num text-4xl font-semibold text-success">{chaosResult.before}</p>
                </div>
                <span className="num pb-2 text-muted-foreground">→</span>
                <div>
                  <p className="label-xs">After</p>
                  <p className="num text-4xl font-semibold text-critical">{chaosResult.after}</p>
                </div>
              </div>
              <Meter value={chaosResult.after} tone="critical" className="mt-4" />
              <p className="mt-2 text-xs text-muted-foreground">
                Drop of {chaosResult.before - chaosResult.after} points from the injected failures.
              </p>
            </Panel>
            ...
            <Panel tone="critical" className="p-4 lg:col-span-2">
              ...
              <p className="mt-2 max-w-2xl text-xs leading-relaxed text-muted-foreground">
                MedCore, BioPack, NorthStar, Aegis and Helix appear independent in the supplier master. All five depend
                on the same capability: <span className="text-foreground">Precision Polymer Certification</span>{" "}
                (CAP-PPC-004), issued by a single accredited lab.
              </p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div className="panel-inset px-3 py-1">
                  <DataRow label="Supplier redundancy" value={`${chaosResult.supplierRedundancy}x`} />
                  <DataRow label="Capability redundancy" value={`${chaosResult.capabilityRedundancy}x`} />
                </div>
                <div className="panel-inset px-3 py-1">
                  <DataRow label="Nodes removed" value={chaosResult.removed.join(", ")} />
                  <DataRow label="Failures injected" value={String(chaosResult.toggles.length)} />
                </div>
              </div>
            </Panel>
          </div>
                  ...
                  <div key={d.id} className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_260px]">
                    ...
                    <div className="panel-inset flex flex-col p-3">
                      ...
                      <div className="mt-3">
                        ...
                        <Meter value={(d.redundancy / d.target) * 100} tone="critical" className="mt-1.5" />
                      </div>
                      <button
                        type="button"
        ...
        <Panel className="p-6">
          ...
          <p className="mt-1.5 max-w-2xl text-xs text-muted-foreground">
            The chaos simulator removes resources from the network, recomputes the resilience score and reports which
            capabilities can still be reconstructed — and which shared dependencies quietly connect resources that look
            independent.
          </p>
        </Panel>
      )}
