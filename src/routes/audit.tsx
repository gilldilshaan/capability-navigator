...
import { DemoTag, PageHeader, Panel, PanelHeader, StatusPill } from "@/components/parallax/primitives";
...
function Audit() {
  const { audit, recoveryStatus, approveRecovery, requestAlternative, rejectRecovery, recommendedPathId, activity } =
    useParallax();
          ...
          <ol className="p-4">
            {audit.map((e, i) => (
              <li key={e.id} className="rise-in relative flex gap-4 pb-5 last:pb-0">
                <span className="num w-11 shrink-0 pt-0.5 text-[11px] text-muted-foreground">{e.time}</span>
          ...
          <div className="border-t border-border p-4">
            {recoveryStatus === "NOT STARTED" ? (
              <p className="text-xs text-muted-foreground">
                No recommendation is pending. Run the disruption analysis to generate recovery paths.{" "}
              ...
              <div className="rise-in flex flex-wrap items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Path {recommendedPathId ?? "C"} · Capability Reconstruction · execution handoff ready.
                  </p>
                </div>
                <Link
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Incident INC-2048 remains open. Increase redundancy, qualify an alternative resource, or develop
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="rounded-sm border border-success/50 bg-success/15 px-3 py-2 font-mono text-[11px] tracking-[0.1em] text-success uppercase transition-colors hover:bg-success/25"
                >
                  Approve recovery
                </button>
                <button
                  ...
                  className="rounded-sm border border-warning/50 bg-warning/12 px-3 py-2 font-mono text-[11px] tracking-[0.1em] text-warning uppercase transition-colors hover:bg-warning/22"
                >
                  Request alternative
                </button>
                <button
                  ...
                  className="rounded-sm border border-critical/50 bg-critical/12 px-3 py-2 font-mono text-[11px] tracking-[0.1em] text-critical uppercase transition-colors hover:bg-critical/22"
                >
                  Reject
                </button>
              </div>
            ...
            {decided ? null : (
              <p className="mt-3 text-[11px] text-muted-foreground">
                Compliance status for Path C: requires human verification of GDP cold-chain sign-off.
              </p>
            )}
          </div>
        </Panel>
            ...
            {activity.map((e) => (
              <div key={e.id} className="border-l border-border py-1.5 pl-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="num text-[10px] tracking-[0.08em] text-info uppercase">[{e.channel}]</span>
                </div>
