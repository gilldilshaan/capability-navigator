...
import { DataRow, DemoTag, KpiCard, PageHeader, Panel, PanelHeader, StatusPill } from "@/components/parallax/primitives";
import { activeDisruption, resilienceTrend } from "@/lib/parallax/data";
...
function Overview() {
  const { resilience, activeDisruptions, redundancy, readiness, recoveryStatus, openIncident, presentation } =
    useParallax();
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
        <Panel tone={activeDisruptions ? "critical" : "success"}>
          <PanelHeader
            ...
            subtitle={activeDisruptions ? "Human-in-the-loop response required" : "Network operating normally"}
            icon={<AlertOctagon className="size-4" />}
            right={
              <StatusPill tone={recoveryStatus === "APPROVED" ? "success" : "critical"}>
                {recoveryStatus === "APPROVED" ? "Recovery approved" : activeDisruption.severity}
              </StatusPill>
            }
          />
          {activeDisruptions === 0 && recoveryStatus === "APPROVED" ? (
            <div className="p-4">
              <p className="text-sm text-foreground">Recovery plan approved — execution handoff ready.</p>
              <p className="mt-1.5 text-xs text-muted-foreground">
                ThermoShield Packaging capability reconstructed via Path C. Network operating normally.
              </p>
              <Link
                to="/audit"
            ...
            <div className="grid gap-5 p-4 md:grid-cols-2">
              <div>
                <h3 className="text-lg font-semibold text-foreground">{activeDisruption.title}</h3>
                <p className="mt-1 text-xs text-muted-foreground">
                  A tier-1 dependency became unavailable. PARALLAX treats this as a lost capability, not a lost supplier.
                </p>
                <button
                  type="button"
              ...
              <div className="panel-inset px-3 py-1">
                ...
                <DataRow label="Dependency" value={activeDisruption.dependency} mono={false} />
                ...
                <DataRow label="Impact" value={activeDisruption.impact} mono={false} />
              </div>
            </div>
          )}
        </Panel>
          ...
          <div className="h-[196px] p-3">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={resilienceTrend} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="score" stroke="var(--color-info)" strokeWidth={1.8} fill="url(#resGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="border-t border-border px-4 py-3">
            <p className="text-xs text-muted-foreground">
              Traditional resilience asks: <span className="text-foreground">do we have a backup supplier?</span>
              <br />
              PARALLAX asks: <span className="text-foreground">how many different ways can we achieve the same outcome?</span>
            </p>
          </div>
        </Panel>
      </div>
