  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Recovery Paths — PARALLAX" },
      {
        ...
        content: "Three ways to achieve the same outcome, scored on speed, risk, cost, capacity and dependency resilience.",
      },
    ],
  }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        right={
          <>
            <StatusPill tone={recoveryStatus === "APPROVED" ? "success" : "warning"}>{recoveryStatus}</StatusPill>
            <DemoTag />
          </>
        }
      />
      ...
      {!pathsGenerated ? (
        <EmptyState
          ...
          actions={
            ...
            >
              {analysis === "running" ? "Analysis running…" : "Run analysis"}
            </button>
          }
        />
      ) : (
        <>
          ...
          <Panel>
              ...
              <table className="w-full min-w-[680px] text-left">
                <thead>
                  <tr className="border-b border-border">
                    {["Path", "Strategy", "Speed 30%", "Risk 25%", "Cost 20%", "Capacity 15%", "Dependency 10%", "Score"].map(
                      (h) => (
            ...
            <p className="border-t border-border px-4 py-3 text-xs text-muted-foreground">
              Illustrative simulation — figures are modelled from demo data, not measured real-world savings.
            </p>
          </Panel>
