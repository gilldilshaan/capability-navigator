  ...
  return (
    <Link to="/" className="group flex items-center gap-2.5">
      ...
      <span className="leading-none">
        <span className="block font-mono text-[15px] font-semibold tracking-[0.28em] text-foreground">PARALLAX</span>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        tone === "default" && "border-border-strong bg-surface-2 text-muted-foreground hover:text-foreground",
      )}
    >
      {icon}
      {children}
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const networkStatus =
    recoveryStatus === "APPROVED" ? "Recovering" : activeDisruptions > 0 ? "Degraded" : "Operational";
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-background/92 backdrop-blur">
        <div className="flex h-14 items-center gap-6 px-5">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <StatusPill tone={networkStatus === "Operational" ? "success" : networkStatus === "Recovering" ? "info" : "warning"}>
              {networkStatus}
            </StatusPill>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <TopBarButton
              ...
              icon={presentation ? <Minimize2 className="size-3.5" /> : <Maximize2 className="size-3.5" />}
            >
      ...
      <div className="flex">
              ...
              {nav.map((item) => {
                ...
                return (
                  <li key={item.to}>
                    ...
                    >
                      <Icon className={cn("size-4", active ? "text-info" : "text-muted-foreground")} />
        ...
        <main className={cn("min-w-0 flex-1 px-5 py-6", presentation && "px-10 py-8")}>{children}</main>
